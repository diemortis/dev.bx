...
export class List
{
	...
	{
		const index = this.items.indexOf(item);
		const content = this.items[index]['title'];
		var result = prompt ('Введите текст', content );
		this.items.push(this.createItem({title: result}));
		this.items.splice(index, 1);
		this.save().then(() => {
			this.renderItems();
		}).catch((error) => {
