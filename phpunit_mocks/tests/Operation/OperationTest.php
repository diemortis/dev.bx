...
//		};
	...
	public function testThatAfterActionFail(): void
	{
		...
		$action = $this->getMockBuilder(\App\Operation\Action::class)
					   ...
					   ->getMockForAbstractClass()
		;
		$errorMessage = 'Error during alter action in test';
	...
	public function testThatAddActionReturnException(): void
	{
		$settings = new App\Operation\Settings();
		...
		$order->expects(static::never())
			  ->method('save')
		;
		$action = $this->getActionThatNeverInvoked();
		...
		$operation ->addAction('wrong path', $action);
	}
